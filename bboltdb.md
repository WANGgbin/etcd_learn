...
为了提高系统的吞吐，读写之间通过快照隔离。即读操作读取历史的某个快照数据，这样就不会让读写操作之间互斥。而在 bbolt 中，这个快照就是 meta。
...
- 性能
    ...
    在 bbolt 中，一次写事务提交就需要持久化到磁盘，性能比较差，所以我们看到 etcd 在使用 bbolt 的时候，是通过将批量的写操作放到一个写事务进行提交的。
